...
private final class LxScriptRunner {
    ...
    private func installContext() {
        ...
        let nativeSend: @convention(block) (String, JSValue?) -> Void = { [weak self] name, payload in
            guard let self else { return }
            self.queue.async {
                self.handleSend(name: name, payload: payload)
            }
        }
        let nativeSetTimeout: @convention(block) (JSValue?, Double) -> Int = { [weak self] callback, delay in
            guard let self, let callback else { return 0 }
            return self.scheduleTimer(callback: callback, delayMilliseconds: delay)
        }
        let nativeClearTimeout: @convention(block) (Double) -> Void = { [weak self] timerID in
            self?.cancelTimer(Int(timerID))
        }
        ...
        jsContext?.setObject(nativeSetTimeout, forKeyedSubscript: "__beansSetTimeout" as NSString)
        jsContext?.setObject(nativeClearTimeout, forKeyedSubscript: "__beansClearTimeout" as NSString)
        ...
        (function(root) {
          ...
          root.setTimeout = function(fn, delay) {
            return root.__beansSetTimeout(fn, Number(delay) || 0);
          };
          root.clearTimeout = function(id) {
            root.__beansClearTimeout(Number(id) || 0);
          };
    ...
    private func scheduleTimer(callback: JSValue, delayMilliseconds: Double) -> Int {
        timerSequence += 1
        let timerID = timerSequence
        let delay = max(0, delayMilliseconds) / 1000
    ...
    private func cancelTimer(_ timerID: Int) {
        guard let work = timers.removeValue(forKey: timerID) else { return }
        work.cancel()
    }
    ...
    private func handleSend(name: String, payload: JSValue?) {
        guard !finished,
              (name.lowercased() == "musicurl" || name.lowercased() == "music_url") else { return }
        ...
        if let string = object as? String {
            ...
            candidate = (dictionary["url"] as? String)
                ...
                ?? (dictionary["playUrl"] as? String)
                ?? (dictionary["audio_url"] as? String)
                ?? (dictionary["music_url"] as? String)
                ?? ((dictionary["data"] as? [String: Any])?["url"] as? String)
                ?? (info?["url"] as? String)
                ?? (info?["play_url"] as? String)
        } else {
            candidate = payload?.toString()
        }
        guard let candidate,
              let url = URL(string: candidate),
