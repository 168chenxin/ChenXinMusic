...
struct ThirdPartySourceImportSheet: View {
            ...
            Task {
                ...
                do {
                    ...
                    if LxScriptRuntime.looksLikeLxScript(text) {
                        ...
                        BeansLogger.shared.log("从链接导入 LX 音源：\(url.absoluteString)", level: .info)
                        BeansHaptics.success()
    ...
    private func parseSources(_ text: String) -> [ThirdPartySource] {
        ...
        for pair in [("{", "}"), ("[", "]")] as [(Character, Character)] {
            ...
            let nested = decodeSources(subData)
            if !nested.isEmpty { return nested }
        }
        return []
    }

    private func decodeSources(_ data: Data) -> [ThirdPartySource] {
        let decoder = JSONDecoder()
        if let list = try? decoder.decode([ThirdPartySource].self, from: data) {
            return list.filter { !$0.template.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        }
        if let single = try? decoder.decode(ThirdPartySource.self, from: data),
           !single.template.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return [single]
        }
        guard let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return [] }
        for key in ["sources", "data", "list", "items", "apis", "plugins"] {
            guard let nested = object[key], JSONSerialization.isValidJSONObject(nested),
                  let nestedData = try? JSONSerialization.data(withJSONObject: nested) else { continue }
            let sources = decodeSources(nestedData)
